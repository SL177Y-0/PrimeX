/**
 * Aries Borrow Modal - Production Component
 * 
 * Complete modal for borrowing assets from Aries Markets
...
import { getAssetByCoinType, toBaseUnits } from '../../../config/ariesAssetsComplete';
import { formatHealthFactor, getHealthFactorColor } from '../../../utils/ariesRiskCalculationsComplete';

interface AriesBorrowModalProps {
  visible: boolean;
  coinType: string | null;
  onClose: () => void;
}

export default function AriesBorrowModal({ visible, coinType, onClose }: AriesBorrowModalProps) {
  ...
  const maxBorrow = useMemo(() => 
    coinType ? getMaxBorrow(coinType) : null,
  [coinType, getMaxBorrow]);
  ...
  const validation = useMemo(() => {
    ...
    if (!portfolio || portfolio.deposits.length === 0) {
      errors.push('No collateral supplied');
    }

    if (maxBorrow && numAmount > maxBorrow.maxAmountDisplay) {
      errors.push('Exceeds maximum safe borrow amount');
    }

    if (reserve && numAmount > parseFloat(reserve.cashAvailable) / (10 ** asset.decimals)) {
  ...
  const simulation = useMemo(() => {
    ...
    return simulateBorrow({
      coinType,
      profileName: 'default',
      amount: baseUnits,
    });
  }, [validation.valid, amount, asset, coinType, simulateBorrow]);

  const handleMaxClick = () => {
    if (maxBorrow) {
      setAmount(maxBorrow.maxAmountDisplay.toFixed(asset?.decimals || 8));
    }
  };
      ...
      >
        ...
        <View style={styles.modalContainer}>
          <View style={styles.header}>
            <Text style={styles.title}>Borrow {asset.symbol}</Text>
          ...
          <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
            ...
            {maxBorrow && (
              <View style={styles.infoCard}>
                <Text style={styles.infoLabel}>Available to Borrow</Text>
                <Text style={styles.infoValue}>
                  {maxBorrow.maxAmountDisplay.toFixed(asset.decimals)} {asset.symbol}
                </Text>
                {reserve.priceUSD && (
                  <Text style={styles.infoValueUSD}>
                    ≈ ${maxBorrow.maxAmountUSD.toFixed(2)}
                  </Text>
                )}
              </View>
            )}
            ...
            <View style={styles.inputSection}>
              <View style={styles.inputHeader}>
                <Text style={styles.inputLabel}>Amount to Borrow</Text>
              </View>
            ...
            {amount && validation.valid && (
              <View style={styles.summaryContainer}>
                ...
                <View style={styles.summaryRow}>
                  <Text style={styles.summaryLabel}>You will borrow</Text>
                ...
                <View style={styles.summaryRow}>
                  <Text style={styles.summaryLabel}>Borrow APR</Text>
          ...
          <View style={styles.footer}>
            <TouchableOpacity
              style={[
                styles.submitButton,
                (!validation.valid || txPending) && styles.submitButtonDisabled
              ]}
